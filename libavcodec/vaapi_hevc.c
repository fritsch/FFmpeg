                                  ...
                                  av_unused uint32_t       size)
{
    ...
    if (!iq_matrix)
        return -1;
    memcpy(iq_matrix->ScalingList4x4    , scaling_list->sl[0]   , sizeof(iq_matrix->ScalingList4x4));
    memcpy(iq_matrix->ScalingList8x8    , scaling_list->sl[1]   , sizeof(iq_matrix->ScalingList8x8));
    memcpy(iq_matrix->ScalingList16x16  , scaling_list->sl[2]   , sizeof(iq_matrix->ScalingList16x16));
    memcpy(iq_matrix->ScalingList32x32  , scaling_list->sl[3]   , sizeof(iq_matrix->ScalingList32x32));
    memcpy(iq_matrix->ScalingListDC16x16, scaling_list->sl_dc[0], sizeof(iq_matrix->ScalingListDC16x16));
    memcpy(iq_matrix->ScalingListDC32x32, scaling_list->sl_dc[1], sizeof(iq_matrix->ScalingListDC32x32));
                                   ...
                                   uint32_t        size)
{
    ...
    slice_param->delta_chroma_log2_weight_denom = sh->chroma_log2_weight_denom;
    ...
    for (list_idx = 0; list_idx < nb_list; ++list_idx) {
        ...
        if (list_idx) {
            slice_param->num_ref_idx_l1_active_minus1 = rpl->nb_refs - 1;
        } else {
            slice_param->num_ref_idx_l0_active_minus1 = rpl->nb_refs - 1;
        }
